import { Hash, Server, Star, Trash2 } from "lucide-react";
    ...
    <div className="space-y-8 py-4 max-w-5xl">
      ...
      {guilds.length === 0 && channels.length === 0 && (
        <div className="rounded-2xl border border-white/10 bg-[#0f0f12]/80 p-8 text-center text-slate-500">
          <p>
            No favorites yet. Add guilds from the Guilds page and channels from
            the Channels page using the star.
          </p>
          <Link
            to="/guilds"
            ...
            {guilds.map((g) => (
              ...
              >
                <button
                  type="button"
                  onClick={() =>
                    navigate("/channels", { state: { guildId: g.id } })
                  }
              ...
              >
                <div className="flex-1 min-w-0">
                  <button
                    type="button"
                    onClick={() =>
                      navigate("/messages", { state: { channelId: c.id } })
                    }
