...
type LogEntry = { id: string; timestamp: string; level: string; kind: string; detail: string; meta: Record<string, any> };
const LEVELS = ["DEBUG", "INFO", "WARNING", "ERROR"];
const KINDS = ["", "tool_call", "server", "export"];
const LEVEL_COLORS: Record<string, string> = { ERROR: "text-red-400 bg-red-950/40", WARNING: "text-yellow-400 bg-yellow-950/40", INFO: "text-blue-300 bg-blue-950/30", DEBUG: "text-slate-500 bg-slate-900/30" };
...
export default function Logs() {
  ...
  const fetchLogs = useCallback(async (opts: { tail?: boolean; after_id?: string } = {}) => {
    ...
    try {
      ...
      setTotal(d.total);
      if (d.entries.length > 0) afterIdRef.current = d.entries[d.entries.length - 1].id;
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      if (afterIdRef.current) fetchLogs({ tail: true, after_id: afterIdRef.current });
      else fetchLogs({ tail: true });
    }, 2000);
    return () => clearInterval(iv);
  }, [tail, fetchLogs]);

  useEffect(() => {
    if (tail && !userScrolled && endRef.current) endRef.current.scrollIntoView({ behavior: "smooth" });
  ...
  const handleExport = async (fmt: string) => {
    ...
    if (level) params.set("level", level); if (kind) params.set("kind", kind); if (search) params.set("search", search);
    const r = await fetch(API_BASE + `/api/logs/export?${params}`);
    const blob = await r.blob(); const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `logs.${fmt}`; a.click(); URL.revokeObjectURL(url);
  };

  const handleClear = async () => {
    await fetch(API_BASE + "/api/logs", { method: "DELETE" });
    setShowClear(false); setEntries([]); setTotal(0);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button className={`h-8 rounded px-3 text-xs font-medium ${tail ? "bg-indigo-600 text-white" : "border border-slate-700 text-slate-400 hover:bg-slate-800"}`}
          onClick={() => setTail(!tail)}>{tail ? "LIVE" : "Tail"}</button>
      ...
      <div ref={containerRef} onScroll={handleScroll}
        ...
        {entries.map((e) => (
          <div key={e.id} className="flex gap-3 py-0.5 hover:bg-slate-900/50 rounded px-1">
            ...
            <span className={`w-16 shrink-0 text-center rounded text-[10px] font-bold ${LEVEL_COLORS[e.level] || "text-slate-400"}`}>{e.level}</span>
