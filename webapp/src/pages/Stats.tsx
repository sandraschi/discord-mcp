...
import { api, type GuildStats, type GuildsResponse } from "../lib/api";
...
export default function Stats() {
  ...
  const pieData = [
    ...
    {
      name: "Offline",
      value: Math.max(0, totalMembers - totalOnline),
      color: "#64748b",
    },
  ].filter((d) => d.value > 0);
  ...
  const handleExportJSON = () => {
    exportJSON(
      { guilds: rows, total_members: totalMembers, total_online: totalOnline },
      "discord-stats.json",
    );
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Member counts and online presence
            </p>
          </div>
        </div>
      ...
      {rows.length > 0 && (
        <>
          ...
          <div className="rounded-2xl border border-white/10 bg-[#0f0f12]/80 overflow-hidden">
            <h2 className="p-4 text-sm font-bold text-slate-300 border-b border-white/10">
              Stats table
            </h2>
