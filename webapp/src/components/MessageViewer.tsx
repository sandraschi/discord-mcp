...
function MessageContent({
  ...
  const cls = compact
    ? "text-slate-400 text-xs truncate max-w-full"
    ...
    <div className="space-y-1">
      {hasRef && (
        <div className="rounded border border-white/10 bg-white/5 px-2 py-1 text-slate-500 text-sm">
          ...
          <span className="ml-1">
            {m.referenced_message?.content?.slice(0, 80) ?? ""}
          </span>
        </div>
      )}
...
function ThreadNode({ m, depth }: { m: Message; depth: number }) {
  return (
    <div
      className={depth > 0 ? "ml-4 mt-2 border-l-2 border-white/10 pl-3" : ""}
    >
      <div className="flex items-baseline gap-2 flex-wrap">
        <span className="font-medium text-amber-400/90">{m.author}</span>
        <span className="text-slate-500 text-xs">
...
function ThreadTree({
  ...
  trees: { root: Message; children: Message[] }[];
}) {
  return (
    <ul className="space-y-4">
      {trees.map(({ root, children }) => (
        <li
          key={root.id}
          className="rounded-xl border border-white/10 bg-[#0f0f12]/50 p-4"
        >
...
function PointCloud({
  ...
  const points = messages.map((m, i) => ({
    ...
    y:
      padding.top +
      (authorIndex(m.author) / Math.max(1, authorList.length)) * innerH * 0.6 +
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0f0f12]/80 overflow-hidden">
      ...
      <svg width={width} height={height} className="overflow-visible" aria-label="Message scatter plot">
        <title>Message scatter plot — X: time, Y: author</title>
        {authorList.map((a, i) => (
          <text
            key={a}
            x={padding.left - 8}
            y={
              padding.top +
              ((i + 0.5) / Math.max(1, authorList.length)) * innerH * 0.6
            }
...
export default function MessageViewer({
  ...
  const foundMsg = pointSelectedId
